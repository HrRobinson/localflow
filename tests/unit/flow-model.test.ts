...
describe('parseFlowGraph — valid graphs', () => {
  ...
  it('accepts a well-typed edge condition', () => {
    const flow = parseFlowGraph(
      graph({ edges: [{ id: 'e1', from: 't', to: 'a', condition: { field: 'x.y', equals: 'bug' } }] })
    )
...
describe('parseFlowGraph — invalid graphs disable loudly, never throw', () => {
  const cases: [string, Record<string, unknown>, RegExp][] = [
    ...
    ['bad node type', graph({ nodes: [{ id: 't', type: 'wormhole', config: {}, position: { x: 0, y: 0 } }] }), /type/i],
    ['duplicate node id', graph({
      nodes: [
        { id: 'dup', type: 'trigger', integration: 'email', ref: 'inbound', config: {}, position: { x: 0, y: 0 } },
        { id: 'dup', type: 'agent', ref: 'claude', config: {}, position: { x: 0, y: 0 } }
      ],
      edges: []
    }), /duplicate/i],
    ['no trigger node', graph({
      nodes: [{ id: 'a', type: 'agent', ref: 'claude', config: {}, position: { x: 0, y: 0 } }],
      edges: []
    }), /exactly one trigger/i],
    ['two trigger nodes', graph({
      nodes: [
        { id: 't1', type: 'trigger', integration: 'email', ref: 'inbound', config: {}, position: { x: 0, y: 0 } },
        { id: 't2', type: 'trigger', integration: 'linear', ref: 'created', config: {}, position: { x: 0, y: 0 } }
      ],
      edges: []
    }), /exactly one trigger/i],
    ['dangling edge target', graph({ edges: [{ id: 'e9', from: 't', to: 'ghost' }] }), /e9.*ghost/i],
    ['dangling edge source', graph({ edges: [{ id: 'e9', from: 'ghost', to: 'a' }] }), /e9.*ghost/i],
    ['orphan node (no inbound)', graph({
      nodes: [
        { id: 't', type: 'trigger', integration: 'email', ref: 'inbound', config: {}, position: { x: 0, y: 0 } },
        { id: 'a', type: 'agent', ref: 'claude', config: {}, position: { x: 1, y: 0 } },
        { id: 'orphan', type: 'action', integration: 'linear', ref: 'comment', config: {}, position: { x: 2, y: 0 } }
      ],
      edges: [{ id: 'e1', from: 't', to: 'a' }]
    }), /orphan/i],
    ['bad integration id', graph({
      nodes: [
        { id: 't', type: 'trigger', integration: 'slack', ref: 'inbound', config: {}, position: { x: 0, y: 0 } },
        { id: 'a', type: 'agent', ref: 'claude', config: {}, position: { x: 1, y: 0 } }
      ]
    }), /integration/i],
    ['ill-typed condition field', graph({
