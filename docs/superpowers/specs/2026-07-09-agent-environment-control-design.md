...
localflow environment, and (B) **workflow watchpoints** that capture state when
...
ships a mature `browser` tool and the **Lobster** workflow engine, so localflow
**integrates** rather than rebuilds.

## Guiding principle

**localflow is the cockpit and the control surface, not the brain.** Panes stay
...
- **OpenClaw owns the brain** — its own sessions/threads (one per ticket/task),
  memory, skills, and Lobster workflows. localflow does **not** reimplement any
  of these.
- **localflow owns the surfaces** — the panes, a control API over them, the
  per-environment operator grant, and a read-only cockpit that reflects what the
  operator is doing.

We do **not** merge the two orchestration models. localflow hosts and observes;
OpenClaw executes.
...
2. **Browser control** — navigate / screenshot / inspect(network, cookies) / act
   ...
   DevTools protocol.
3. **OpenClaw integration + cockpit** — a `localflow` skill on the OpenClaw side
...
Terminal control is **nearly free**: localflow already exposes `write(id, data)`
...
- exposes the endpoint+token to the operator (the OpenClaw `localflow` skill is
  configured with them).

### Connection

OpenClaw runs either as a localflow-managed session in the environment (localflow
...
link is the grant's endpoint+token — localflow does not care where OpenClaw runs,
only that a holder of the token drives panes in the granted environment.

### Scoping (the isolation guarantee)

Every pane has a **stable localflow handle**. The control API resolves handles
...
### The control API (localflow-exposed, loopback only)
...
On the OpenClaw side these are wrapped by a shipped **`localflow` skill** (portable
skill format), so the operator calls `localflow.panes`, `localflow.screenshot`,
`localflow.prompt`, etc., as ordinary skills/tools.
...
localflow-owned control surface is cleaner. Revisit if OpenClaw's richer browser
...
workflow — the `localflow` skill's `checkpoint` action:

1. The user registers a watch in localflow (in-memory) against a workflow + step
...
2. When the workflow reaches that step, the `checkpoint` action runs: it reads the
   current state, POSTs it to localflow `/captures`, and **optionally halts** on
   Lobster's approve token for human review.
3. localflow stores the capture and surfaces it in the cockpit; if halted, the
...
reads and saves state.* localflow never runs Lobster (OpenClaw does, in-process);
it only registers the watch, receives the capture, and drives the review UI.

## The cockpit

A read-only view in localflow, per environment:
...
2. Operator: `localflow.screenshot(handle = shopA-browser)` → localflow writes the
   capture into shop A's cwd, returns the path.
3. Operator: `localflow.prompt(handle = shopA-terminal, text = ticket + path)` →
   localflow writes it to the pty; Claude Code makes the change; `localhost` reloads.
4. Operator: `localflow.screenshot(shopA-browser)` again to verify; updates the ticket.
...
localflow supplied the panes, the control surface, and the cockpit. OpenClaw
supplied the brain.
...
- A shipped **`localflow` skill** wrapping the control API (`panes`, `navigate`,
...
- Reimplementing OpenClaw's sessions, memory, skills, or Lobster in localflow.
...
- Non-OpenClaw operators — the control API is generic, but v1 targets OpenClaw and
  ships only the OpenClaw `localflow` skill.
- Persisting the localflow activity feed across restarts (separate concern).
...
These are OpenClaw-facing details to confirm during planning; the localflow design
above is stable regardless of their resolution:

- Exact OpenClaw skill packaging/config for pointing the `localflow` skill at a
  grant's endpoint+token.
- Whether the `checkpoint` watchpoint is authored directly in the Lobster workflow
  or injected/registered declaratively by localflow at a labeled step.
- Whether browser control stays fully localflow-implemented (v1 choice) or later
