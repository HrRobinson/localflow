...
**Tech Stack:** Electron main/preload/renderer, React 19, xterm, Tailwind v4 (`@theme` tokens are real runtime-overridable `:root` custom properties), vitest (`tests/unit/**`, node env), Playwright `_electron` e2e (`launchApp`, `LOCALFLOW_E2E`).
...
- Produces:
  ...
  - `LocalflowApi.setKeybinding(action: KeyAction, binding: string): Promise<Record<KeyAction, string> | null>`, `.resetKeybinding(action: KeyAction): Promise<Record<KeyAction, string>>`, `.resetAllKeybindings(): Promise<Record<KeyAction, string>>`, `.onKeybindingsChanged(cb: (bindings: Record<KeyAction, string>) => void): () => void`.
...
**Interfaces:**
- Consumes: `window.localflow.getKeybindings` / `setKeybinding` / `resetKeybinding` / `resetAllKeybindings` / `onKeybindingsChanged` (Task 1); `serializeKeyEvent`, `findConflicts`, `KeyAction` (Task 1).
...
export default function KeybindingsEditor(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent): void => {
      ...
      setCapturing(null)
      setConflict(null)
      void window.localflow.setKeybinding(action, combo).then((next) => {
...
- Produces:
  ...
  - `LocalflowApi.setDefaultAgent(agentId: AgentId): Promise<AgentInfo[] | null>`, `.setAgentOverride(agentId: AgentId, override: AgentOverride): Promise<AgentInfo[] | null>`.
...
- Produces:
  ...
  - `LocalflowApi.getTheme(): Promise<{ name: string; theme: Theme; error?: string }>`, `.listThemes(): Promise<string[]>`, `.setTheme(name: string): Promise<{ name: string; theme: Theme; error?: string }>`, `.openThemesFolder(): void`, `.onThemeChanged(cb: (payload: { name: string; theme: Theme; error?: string }) => void): () => void`.
...
- Modify: `README.md`
- Modify: `docs/superpowers/specs/2026-07-06-localflow-v2-roadmap.md`

**Interfaces:**
- Consumes: everything above; DOM contracts `.kb-row[data-action]`, `.kb-capture`, `.agent-default[data-agent]`, `.theme-select`, `.landing select`; existing `launchApp` (with its `agentPaths` param), `LOCALFLOW_E2E`.
...
2. `docs/superpowers/specs/2026-07-06-localflow-v2-roadmap.md` — § M4: prepend a line noting the shipped design and the dropped provider tokens:
...
- config.json unknown-key `extra` preservation: `defaultAgent`/`agents`/`theme` are made KNOWN typed keys (added to `KNOWN_TOP_LEVEL_KEYS`) because localflow reads AND writes them via the GUI; `environments` names stay on `extra` (read-only). A regression test asserts `environments` still round-trips on `extra` alongside the new keys, and that `setDefaultAgent`/`setAgentOverride`/`setTheme` preserve it.
...
- Env override precedence is `process.env < injection.env < user override`; documented that hook vars are localflow-internal so real collisions don't occur — if a user deliberately shadows a hook var they accept a degraded status feed.
