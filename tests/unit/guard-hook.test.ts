...
const base: ResolvedGuard = {
  bin: '/Application Support/saiife/saiifeguard',
  auditLog: '/Application Support/saiife/guard-audit.jsonl',
  packs: [],
  seenDir: '/Application Support/saiife/guard-seen'
}

describe('guardHookCommand', () => {
  it('single-quotes the binary and audit path (spaces safe)', () => {
    const cmd = guardHookCommand(base, 'pane1')
    expect(cmd).toContain(`'/Application Support/saiife/saiifeguard' check --hook-exit`)
    expect(cmd).toContain(`--audit-log '/Application Support/saiife/guard-audit.jsonl'`)
    expect(cmd).toContain('--audit-tag pane1')
  })

  it('threads --seen-dir, single-quoted (path-with-spaces safe)', () => {
    const cmd = guardHookCommand(base, 'pane1')
    expect(cmd).toContain(`--seen-dir '/Application Support/saiife/guard-seen'`)
