...
- Produces:
  ...
  - `SaiifeApi.gitStatus(id): Promise<GitStatus>`, `.gitDiff(id, path, staged): Promise<DiffResult>`.
...
- Produces:
  ...
  - `SaiifeApi.getCapabilities(): Promise<Capabilities>`, `.openLazygit(id): Promise<SessionInfo | null>`, `.openEditor(id): Promise<boolean>`.
...
```ts
  ...
  ipcMain.handle('git:capabilities', async (): Promise<Capabilities> => {
    ...
    const [lazygitPath, editorPath] = await Promise.all([
      resolveTool('lazygit', process.env['SAIIFE_LAZYGIT_BIN']),
      resolveTool(editorBin, process.env['SAIIFE_EDITOR_BIN'])
    ])
    capsCache = {
  ...
  ipcMain.handle('git:openLazygit', async (_e, id: string) => {
    ...
    const lazygitPath = await resolveTool('lazygit', process.env['SAIIFE_LAZYGIT_BIN'])
  ...
  ipcMain.handle('git:openEditor', async (_e, id: string) => {
    ...
    const resolved = await resolveTool(bin, process.env['SAIIFE_EDITOR_BIN'])
...
Add to `SaiifeApi`, after `gitDiff`:
...
`SAIIFE_CLAUDE_BIN`), pointing lazygit/editor at nonexistent absolute paths so
...
2. `docs/superpowers/specs/2026-07-06-saiife-v2-roadmap.md`, § M6: prepend a line
...
- *Escape-hatch resolution vs the e2e login-shell hazard.* `whichViaLoginShell` spawns
  ...
  absolute paths). `resolveTool` honors `SAIIFE_LAZYGIT_BIN` / `SAIIFE_EDITOR_BIN`
